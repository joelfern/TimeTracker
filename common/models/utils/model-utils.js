...
/**
 * Generates a random 6 digit reference error for log debugging
 */
var generateReferenceNumber = function() {
	// our reference number is only number characters
	var numbers = '0123456789';
	// each reference number is 6 numbers long
	var refNumberLength = 6;
	// init the number
	var refNumber = '';
	// generate a random number character 6 times and concat them to generate our random number string
	for (var i = 0; i < refNumberLength; i++) {
		var rnum = Math.floor(Math.random() * numbers.length);
		refNumber += numbers.substring(rnum, rnum + 1);
	}
	return refNumber;
};
