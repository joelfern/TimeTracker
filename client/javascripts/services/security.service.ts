...
export class SecurityService {
	...
	private userIsContributor: boolean;
	private userIsApprover: boolean;
	...
	login = (username: string, password: string) => {
		return this.http.post(
			...
			.then((res) => {
				...
				// Redirects via window location if returning to /approval
				// This is a patch fix for nav breaking on approval page
				// TODO - Resolve nav breaking when navigating to and from approval
				if (this.loginReturnPath === '/approval') {
					window.location.pathname = '/page/approval';
				} else {
	...
	currentEndUser = () => {
		...
		return this.http.get(
			...
			.then((res) => {
				...
				// reset the user roles to false
				this.userIsAdmin = this.userIsApprover = this.userIsContributor = false;
				if (this.endUser.roles) {
					for (var role of this.endUser.roles) {
						if (role.name === 'admin') this.userIsAdmin = true;
						if (role.name === 'contributor') this.userIsContributor = true;
						if (role.name === 'approver') this.userIsApprover = true;
					}
				}
			})
			.catch(error => {
	 ...
	 * @param contributor - boolean whether or not the page requires contributor access
	 * @param approver - boolean whether or not the page requires approver access
	 * @param admin - boolean whether or not the page required admin access
	 */
	checkAccessPrivileges = (contributor: boolean, approver: boolean, admin: boolean) => {
		if (!this.endUser) {
			this.currentEndUser()
				.then(response => {
					this.checkPrivilegesHelper(contributor, approver, admin);
				})
		} else {
			this.checkPrivilegesHelper(contributor, approver, admin);
		}
	 ...
	 */
	checkPrivilegesHelper = (contributor: boolean, approver: boolean, admin: boolean) => {
		// check that the user has the appropriate access privileges, otherwise return them to home
		if ((contributor && !this.userIsContributor) ||
			(approver && !this.userIsApprover) ||
	...
	/**
	 * Returns whether or not signed in user is a contributor
	...
	/**
	 * Returns whether or not a signed in user is an approver
	 * @return userIsApprover
	 */
	getUserIsApprover = () => {
		return this.userIsApprover
	}

	/**
	 * Sets whether or not the signed in user is a contributor
	 * @param contributor - boolean whether or not the user is a contributor
	 */
	setUserIsContributor = (contributor: boolean) => {
		this.userIsContributor = contributor;
	}

	/**
	 * Sets whether or not the signed in user is an approver
	 * @param approver - boolean whether or not the user is an approver
	 */
	setUserIsApprover = (approver: boolean) => {
		this.userIsApprover = approver;
	}
};
